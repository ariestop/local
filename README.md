...
2. **Развернуть базу данных**
   ...
   - На первом запуске скрипт создаёт БД, импортирует `public/infosee2_m2sar.sql` и применяет миграции.
   - На последующих запусках скрипт пропускает baseline-импорт и применяет только новые миграции из `migrations/`.
   - При необходимости имя/путь дампа можно изменить через `.env`: `DB_DUMP_PATH=public/your_dump.sql`
...
6. **SEO (Yandex-friendly)**
   ...
   - `robots.txt` расположен в `public/robots.txt` и включает `Sitemap: /sitemap.xml`.
