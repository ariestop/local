...
1. **Composer** (опционально, для .env и логирования)
   ...
   composer install
   ```
   Если глобального `composer` нет, используйте:
   ```bash
...
3. **Веб-сервер**
   ...
   - Для OpenServer/OSP в `.osp/project.ini` рекомендуется:
...
4. **PHP Debug Bar** (опционально, только для разработки)
   - Установка: `composer install` / `php composer.phar install` (подключит php-debugbar как dev-зависимость)
   - В `.env` укажите `APP_ENV=dev` — панель появится внизу страницы
   - При `APP_USE_FRONT_CONTROLLER_URLS=1` ассеты Debug Bar отдаются через `/index.php/debugbar/*`
...
- Тестовый gate ужесточён: `phpunit.xml` падает на `PHPUnit notice/deprecation` (`failOnPhpunitNotice`, `failOnPhpunitDeprecation`).
...
- **Добавление объявления** — только для авторизованных, до 10 фото (форматы загрузки: JPEG/PNG, до 5 МБ на файл)
